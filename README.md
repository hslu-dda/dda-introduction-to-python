# DD+A Introduction to Python

Course material for the Introduction to Python taught at Data Design + Art, HSLU. 

### Think 3 Introduction Course:

1. [Setup](./01_Setup/) and [PyEnv Cheatsheet](./01_Setup/Pyenv-Cheatsheet.md)
2. [Fundamentals](./02_Fundamentals/)
3. [Pandas](./03_Pandas/)
4. [Visualization with Vega Altair](./04_Visualization_Vega_Altair/)

### Further Python related ressources:

- [Key syntax differences between Python and JavaScript](./Ressources/Differences_to_JavaScript/)
- [Working with Geodata using Geopandas](.R/essources/Geopandas/)
- [Calculating correlations using Pandas](./Ressources/Correlations)
- [Applying Natural Language Processing to analyze unstructured text](./Ressources/Natural_Language_Processing)
- [Creating random samples with representative data distribution using Stratification](./Ressources/Stratafication)
- [No more ugly charts! Create your Vega Altair Theme](./Ressources/Vega_Altair_Theme)
- [Learn about the Zen of Python and how complicated Love can be](./Ressources/The_Zen_of_Python)

### Older Semesters Archive

- [HS25_Archived](./HS25_Archived/)